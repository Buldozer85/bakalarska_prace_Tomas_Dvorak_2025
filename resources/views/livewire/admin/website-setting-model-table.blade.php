<div class="overflow-x-auto max-w-[1400px] mx-6 xl:mx-auto space-y-4" x-data="{
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 shadow-md sm:rounded-lg">
        ...
        <tr>
            ...
            <x-admin.table-header column="key" :sort-direction="$sortDirection" :sort-by="$sortBy">
                Klíč
            </x-admin.table-header>

            <x-admin.table-header column="value" :sort-direction="$sortDirection" :sort-by="$sortBy">
                Hodnota
            </x-admin.table-header>
        ...
        <tbody>
        @foreach($data as $websiteSetting)
                ...
                <td class="px-6 py-4">
                    <div class="flex flex-row gap-x-4 items-center">
                        <div class="flex flex-row items-center gap-x-2">
                            <a href="{{ route('administration.websiteSetting.detail', $websiteSetting->id) }}" class="font-medium text-brand">Upravit</a>
        ...
        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
            ...
            <td class="px-6 py-4">
                <x-admin.form.input wire:model.live="key" type="text" name="key" id="key" placeholder="Klíč"></x-admin.form.input>
            </td>
            <td class="px-6 py-4">
                <x-admin.form.input wire:model.live="value" name="value" id="value" placeholder="Hodnota"></x-admin.form.input>
            </td>
    ...
    <x-admin.modal id="deleteWebsiteSettingModal">
        <div class="flex flex-col justify-center items-center gap-y-6">
            ...
            <p class="font-normal">Opravdu si přejete smazat nastavení:  <span x-text="model.name"></span>?</p>

            <div class="flex flex-row gap-x-4 w-full">

                <x-admin.button data-modal-hide="deleteWebsiteSettingModal" class="flex-1" @click="$wire.delete(model.id)" action="submit" type="danger">Smazat</x-admin.button>
                <x-admin.button data-modal-hide="deleteWebsiteSettingModal" class="flex-1" type="yellow">Zrušit</x-admin.button>
            </div>
        </div>
    </x-admin.modal>
</div>
