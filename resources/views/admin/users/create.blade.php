    ...
    <div class="shadow-lg rounded-md p-12 space-y-12 max-w-[1400px] mx-auto">
        ...
        <form class="space-y-4" method="POST" action="{{ route('administration.users.user.create') }}">
            ...
            <div class="grid grid-cols-2 gap-4 gap-x-8 border-b border-b-gray-200 pb-12">
                <x-admin.form.input id="first_name" name="first_name" label="Jméno"/>
                <x-admin.form.input id="last_name" name="last_name" label="Příjmení"/>
                <x-admin.form.input id="email" name="email" label="E-mail"/>
                <x-admin.form.input id="phone" name="phone"  label="Telefon"/>
                <x-admin.form.select id="role" name="role" label="Role" :options="\App\Enums\Roles::options()"/>
                <x-admin.form.select id="email_verified_at" name="email_verified_at" :selected="2"  label="Ověřený e-mail" :options="['Ne', 'Ano', 'Ano/Ne']"/>
            </div>
