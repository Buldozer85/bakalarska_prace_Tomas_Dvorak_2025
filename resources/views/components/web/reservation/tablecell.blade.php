...
<td @if(in_array($type, ['empty', 'selected'])) {{ $attributes->wire('click') }} @endif  @if($type !== 'empty') data-tooltip-target="{{ $id }}"  @endif class="px-6 py-4 border overflow-hidden {{ $style }} @if(!$readOnly && in_array($type, ['empty', 'selected'])) cursor-pointer @endif">
{{ $slot }}
    @if($type !== 'empty')
